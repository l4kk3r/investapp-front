...
const BrokerProfile = () => {
    ...
    const toArchive = (id) => {
        ...
        axios.post("http://localhost:5500/api/post/archive", {id, archived: true}).then(response=>console.log(response.data))
    }
    ...
    useEffect(() => {
        if (state) {
            axios.post("http://localhost:5500/api/post/users", {creator_id: state.id, archived: false}).then(res=>{setPosts(res.data.posts); setAnswers(res.data.answers); console.log(res.data)}) }
    }, [state])
